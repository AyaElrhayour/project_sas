...
char titre[100] ,description[600], status[10];
    ...
    printf("Liste des taches :\n");
     for (i = 0; i < taskCount; i++) {
        remainingTime = calculateDeadline(
            ...
            tasks[i].deadline.tm_mon + 1, 
            tasks[i].deadline.tm_year + 1900, 
        ...
        if (remainingTime < 0) {
            ...
            printf("[%d] Titre: %s Description: %s  Remaining time: %d hours  statu of the task : %d \n", i + 1, tasks[i].titre, tasks[i].description, remainingTime);
        }
    ...
    do {
        ...
        switch (choice) {
            ...
            for (i = 0; i < n; i++)
            {
                ...
                scanf("%d", &status);
                addTask(tasks, &taskCount, description, titre, deadline , status);
            }
                break;
            case 2:
